...
def synthesizer(summaries, topics, length):
    ...
    resp = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": f"You are a podcast host, this podcast is a daily {length} minute news breakdown on these topics: {topics}, similar in format to the popular npr show 'the daily'."},
            {"role": "user", 
